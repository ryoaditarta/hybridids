...
MODEL_DIR = f"./{MODEL_NAME}" # Direktori unik untuk setiap model

# --- Konfigurasi Data ---
TRAIN_FILE_PATH = "/home/victim/project/data/csv/slowread.csv"
EVAL_FILE_PATH = "/home/victim/project/src/data/csv/slowreadvalidation_balanced.csv"
ATTACK_IP = "192.168.100.146"
...
def train_model(params, preprocessed_data,scaler):
    ...
    autoencoder.save(os.path.join(params['dir'], "slowreadautoencoder.h5"))
    joblib.dump(scaler, os.path.join(params['dir'], "slowreadscaler.gz"))
...
def define_threshold(val_mse):
    ...
    return np.quantile(val_mse, 0.75)
...
if __name__ == "__main__":
    ...
    # Preprocess data sekali di awal
    ...
    if preprocessed_data is None:
        ...
        for run in range(1, num_runs + 1):
            print(f"\n======================== RUN {run} dari {num_runs} ========================")
            trained_model, trained_scaler, val_errors = train_model(params, preprocessed_data, scaler)
            threshold = define_threshold(val_errors)
            thresholds.append(threshold)
            print(f"[RUN {run}] Threshold: {threshold:.6f}")

        avg_threshold = np.mean(thresholds)
        std_threshold = np.std(thresholds)
        print("\n" + "="*80)
        print(f"[INFO] ✅ Threshold rata-rata (85 percentile) dari {num_runs} run: {avg_threshold:.6f}")
        print(f"[INFO] 📊 Standar deviasi threshold: {std_threshold:.6f}")
        print("="*80)
